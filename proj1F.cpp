...
void multiplyVector(double *a, double b, double *result, int size){
	for(int i = 0; i < size; i++){
		result[i] = (a[i] * b);
	}
}

void calculateShading(Triangle *t, LightingParameters lp, double *O){
		for(int i = 0; i < 3; i++){
			//printf("Vertex: %d\n", i);
			double L[3]; L[0] = lp.lightDir[0]; L[1] = lp.lightDir[1]; L[2] = lp.lightDir[2];
			double copyXYZ[3]; copyXYZ[0] = t->X[i]; copyXYZ[1] = t->Y[i]; copyXYZ[2] = t->Z[i];
			double v[3];
			subtract_vectors(O, copyXYZ, v);
			double norm[3]; 
			norm[0] = t->normals[i][0]; norm[1] = t->normals[i][1]; norm[2] = t->normals[i][2]; 

			normalize(L); normalize(v); normalize(norm);
			double ln = dot_product(norm, L, 3);
			//printf("LdotN: %f\n", ln);
			...
			double temp[3]; 
			double R[3];
			multiplyVector(norm, 2*dot_product(norm, L, 3), temp, 3);
			subtract_vectors(temp, L, R);
			normalize(R);
			double a = dot_product(v, R, 3);
			//printf("rdotV: %f\n", a);
			//printf("reflectionVector: %f, %f, %f\n", R[0], R[1], R[2]);
			double spec = pow(max(a, 0.0), lp.alpha);
			double shadingAmount =  (lp.Ka + (lp.Kd * diffuse) + abs(lp.Ks *(spec)));
			//printf("diffuse: %f\n", lp.Kd * diffuse);
			//printf("Specular comp: %f\n", spec);
			//printf("Value: %f\n", shadingAmount);
			t->shading[i] = shadingAmount;
		}
}

void assignPixels(Image *img, int colMin, int colMax, 
int rowMin, UNUSED int rowMax, double *leftEndColor, double *rightEndColor, 
double leftDepth, double rightDepth, double rightEnd, double leftEnd, double leftShade, double rightShade){
	//for(int x = rowMin; x<=rowMax; x++){
		for(int y = colMin; y<=colMax; y++){
			...
			double currShade =  calculateDepth(leftShade, rightShade, leftEnd, rightEnd, y);
		    //printf("currentShade: %f\n", currShade);
			R =   min(1.0, R *currShade); G =  min(1.0, G*currShade); B = min(1.0,  B*currShade); 
			/*printf("====\n");
...
Image *drawImage(Camera c){
	...
	for(int t = 0; t<tl->numTriangles; t++){
		...
		calculateShading(&(tl->triangles[t]),lp, c.position);
		...
		double leftEndShade; double rightEndShade;
		...
		for (int i = C441(rowMin); i <= F441(rowMax); i++){
			...
			if(i>= 0 && i < 1000){
				 if(i < tl->triangles[t].Y[midVertex]){
					...
					//rightEndShade = calculateDepth(tl->triangles->shading[minY], tl->triangles[t].shading[maxY], tl->triangles[t].Y[minY], tl->triangles[t].Y[maxY], i);
					 leftEndShade= findenddepth(tl->triangles[t].Y[maxY], tl->triangles[t].shading[maxY], tl->triangles[t].Y[minY], tl->triangles[t].shading[minY], i);
					...
					rightEndShade =  findenddepth(tl->triangles[t].Y[midVertex], tl->triangles[t].shading[midVertex], tl->triangles[t].Y[minY], tl->triangles[t].shading[minY], i);
					...
					//rightEndShade = calculateDepth(tl->triangles->shading[maxY], tl->triangles[t].shading[minY], tl->triangles[t].Y[maxY], tl->triangles[t].Y[minY], i);
					...
					leftDepth = findenddepth(tl->triangles[t].Y[maxY], tl->triangles[t].Z[maxY], tl->triangles[t].Y[minY], tl->triangles[t].Z[minY], i);
					//leftEndShade = calculateDepth(tl->triangles->shading[maxY], tl->triangles[t].shading[midVertex], tl->triangles[t].Y[maxY], tl->triangles[t].Y[midVertex], i);


					rightEndShade= findenddepth(tl->triangles[t].Y[maxY], tl->triangles[t].shading[maxY], tl->triangles[t].Y[midVertex], tl->triangles[t].shading[midVertex], i);
					leftEndShade =  findenddepth( tl->triangles[t].Y[minY], tl->triangles[t].shading[minY],tl->triangles[t].Y[maxY], tl->triangles[t].shading[maxY], i);
            ...
            //^^uncomment for going down triangles
				if(leftEnd > rightEnd){
					...
					temp = rightEndShade; rightEndShade = leftEndShade; leftEndShade = temp;
				...
				//printf("leftR: %f, leftG: %f, leftB: %f\n", leftEndColor[0], leftEndColor[1], leftEndColor[2]);
			//printf("rightR: %f, rightG: %f, rightB: %f\n", rightEndColor[0], rightEndColor[1], rightEndColor[2]);
				//printf("leftShade %f, rightShade %f\n", leftEndShade, rightEndShade);
				//printf("leftShade: %f rightShade %f\n", leftEndShade, rightEndShade);
				assignPixels(im, C441(leftEnd), F441(rightEnd), i, i, 
				leftEndColor, rightEndColor,
				leftDepth, rightDepth, rightEnd, leftEnd, leftEndShade, rightEndShade);
			}else{
				continue;
			}
		}
...
int main(){
	for(int x = 0; x < 1; x++){
		...
		sprintf(fileName, "proj1F_frame%04d.pnm", x);
